...
def initTheme():
    ...
    with dpg.theme(tag="theme_title_text"):
        with dpg.theme_component(dpg.mvText):
            dpg.add_theme_color(dpg.mvThemeCol_Text, (225, 230, 242, 255))
...
def createTitleContent():
    ...
    with dpg.window( label="Title", tag="title_window", min_size=[10, 10], no_title_bar=True, no_move=True, no_resize=True, no_collapse=True, no_scrollbar=True, no_bring_to_front_on_focus=True, pos=[0, 0], width=total_w, height=TITLE_H):
        dpg.add_text("TimeiT", tag="ttl_name", pos=[(vw / 2) - 20, 2])
        dpg.bind_item_theme("ttl_name", "theme_title_text")
        dpg.bind_item_font("ttl_name", "font_title")
...
def createClockContent():
    with dpg.window(label="Clock", tag="clock_window", no_title_bar=True, pos=[RIGHT_X, CONTENT_Y], width=RIGHT_W, height=CONTENT_H):
        dpg.add_text(datetime.now().strftime("%H:%M:%S"), tag="clock", pos=[(RIGHT_W / 2) - 270, (CONTENT_H / 2) - 80])
...
def createFunctionContent():
    with dpg.window(label="Function", tag="function_window", no_title_bar=True, pos=[0, CONTENT_Y], width=LEFT_W, height=CONTENT_H):
        ...
        dpg.add_button(label="Pomodoro", tag="pomo_button", callback=on_tab_change, width=LEFT_W, height=btn_h)
        dpg.bind_item_theme("pomo_button", "theme_accent_button")
